...
def prepare_workflow_monitor(success, document=None):
    ...
    if success and document is not None:
        ...
        data = {
            "doc": {
                "runStatus": "success" if success else "failed",
                "lastRunTime": int(math.floor(time.time()))
            }
        }
    return data
        ...
        if post_data['workflow'] is not None and post_data['workflow']['_source'] is not None:
            ...
            for workflowAction in post_data['workflowActions']:
                ...
                for yarnJobInfo in workflowAction['yarnJobs']:
                    ...
                    #task Information
                    if yarnJobInfo['taskInfo'] is not None:
                        for taskInfo in yarnJobInfo['taskInfo']:
                            document_str = document_str + '{"index": {}}\n' + json.dumps(taskInfo) + '\n'

                    #task Attempt Info
                    if yarnJobInfo['taskAttemptsCounters'] is not None:
                        for taskAttempt in yarnJobInfo['taskAttemptsCounters']:
                            document_str = document_str + '{"index": {}}\n' + json.dumps(taskAttempt) + '\n'

                    # time point Task counts
                    if yarnJobInfo['tpTaskStats']:
                        document_str += generate_bulk_data(yarnJobInfo['tpTaskStats'], type="taskCounts")

            #logger.debug("Post Data Output string %s " % document_str)
            return document_str
