...
def get_job_info(job_history_server, job_id, wfName, wfId, wfaId, wfaName):
...
def get_task_info(job_history_server, job_id, wfName, wfId, wfaId, wfaName):
    location = job_history_server['host']
    port = job_history_server['port']
    path = '/ws/v1/history/mapreduce/jobs/{0}'.format(job_id)
...
def get_app_info(timeline_server, app_id, wfName, wfId, wfaId, wfaName):
...
def get_task_ids_by_job(job_history_server, job_id):
    location = job_history_server['host']
    port = job_history_server['port']
    path = '/ws/v1/history/mapreduce/jobs/{0}/tasks'.format(job_id)
    json_resp_tasks = http_request(location, port, path, scheme=job_history_server['scheme'])
...
def get_container_info(timeline_server, job_id):
...
def get_taskattempt_container_info(job_history_server, timeline_server, job_id, task_ids, wfName, wfId, wfaId, wfaName):
    ...
    try:
        ...
        #containers_list = get_container_info(timeline_server, job_id)
        containers_list = []
...
def get_containers_node(app_id):

    location = resource_manager['host']
    port = resource_manager['port']
    path = '/ws/v1/cluster/nodes'.format(app_id)
    nodes_json = http_request(location, port, path, resource_manager['scheme'])
...
def get_stage_attempt_ids(app, app_attempt):
    ...
    if all_stages_json is None:
        return None
    stage_ids = [{'stageId': s['stageId'], 'stageAttemptId': s['attemptId'], 'numTasks' : s['numTasks'] if 'numTasks' in s else s['numCompleteTasks'] + s['numFailedTasks'] } for s in all_stages_json]
    return stage_ids
