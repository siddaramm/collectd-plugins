...
def get_critical_path(jobs_list, startTimeKey, endTimeKey):
    ...
    #collectd.debug("sorted_job_list: {0}".format(sorted_job_list))
    if sorted_job_list and len(sorted_job_list) > 1:
        ...
        for job in sorted_job_list:
            if job[startTimeKey] and job[endTimeKey] and job[startTimeKey] > 0 and job[endTimeKey] > 0:
                ...
                #collectd.debug("IterationStartTime:{0}, IterationEndTime:{1}".format(iterationStartTime, iterationEndTime))
                #collectd.debug("JobStartTime:{0}, JobEndTime:{1}".format(job[startTimeKey], job[endTimeKey]))
                if job[startTimeKey] >= iterationStartTime and job[startTimeKey] <= iterationEndTime:
                    ...
                    #collectd.debug("Added to critical path iterationStartTime:{0} iterationEndTime:{1}".format(iterationStartTime,
                                                                                                    #iterationEndTime))
                    iterationStartTime = job[startTimeKey]
                    iterationEndTime = job[endTimeKey]
                index = index + 1
